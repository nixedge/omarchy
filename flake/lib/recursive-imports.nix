    ...
    _self: lib: {
      recursiveImports =
        let
          ...
          files =
            ...
            if lib.pathType path == "directory" then
              lib.collect lib.isString (
                lib.mapAttrsRecursive (path: _type: lib.concatStringsSep "/" path) (getDir path)
              )
            else
              [ path ];
          ...
          validFiles =
            path:
            map (file: if lib.hasPrefix "/nix/store" file then file else path + "/${file}") (
              lib.filter (lib.hasSuffix ".nix") (files path)
            );
        in
